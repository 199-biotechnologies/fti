...
Those metrics are not in dispute. The question is different. Takahashi measures learned structure and control capacity per joule. Fagan measures causal or goal-directed _work_ per unit irreversible information --- his numerator is outward causal efficacy, not inward model quality. Perrier measures energy efficiency relative to a behavioural intelligence score. Each metric is physically meaningful. None, however, makes _adaptively useful internal structure itself_ the explicit numerator. The unresolved definitional question is therefore not whether intelligence is physically costly. It is which internal distinctions count as intelligence-relevant rather than merely ordered or merely benchmark-effective.

The usefulness filter belongs at the centre. Kolchinsky and Wolpert @kolchinsky2018 have already formalised semantic information as the correlations causally necessary for maintaining existence, and the information bottleneck @tishby1999 already optimises relevance-filtered compression. The contribution here is not the filter itself but the _combination_: a relevance-filtered structure term ($C_u$), a thermodynamic acquisition-efficiency term ($I_"eff"$), and a behavioural axis ($A$) evaluated jointly rather than collapsed to a single scalar.
...
Let $M_t$ denote the system's internal state or internal model at time $t$. Let $H_t$ denote the system's past sensorimotor history up to but not including $t$; specifically, $H_t$ is the raw sensorimotor trajectory and does not include learned parameters, weights, or compressed summaries that constitute $M_t$. Let $Z_(t+tau)^V$ denote those future environmental variables or latent states whose prediction or control matters for the relevant viability or goal criterion, evaluated at a horizon $tau$ chosen to match the system's decision timescale. A concrete candidate for useful internal structure is then
...
Still and colleagues support the predictive side of the proposal: nonpredictive information stored by a driven system provides a stepwise lower bound on dissipated work, $chevron.l W_"diss" chevron.r gt.eq k_B T I_"nonpred"$ @still2012. That inequality is an anchor for the claim that arbitrary stored complexity should not appear in the numerator; it does not, however, set an equality for finite protocols with additional irreversibility sources. It does _not_ by itself derive intelligent control. Control enters through agency-sensitive quantities such as empowerment @klyubin2005 and through closed-loop frameworks such as active inference @friston2010 @pezzulo2024a. Prediction and control are therefore treated as complementary requirements. Still supports the filter against useless memory; empowerment and active inference support the requirement that retained structure must also be deployable in action.
...
Several observations follow. First, $C_u$ isolates the useful information in this toy: both the rule learner and the lookup table carry one bit about $Z$, but the lookup table wastes an additional bit on nuisance structure. A raw information metric would credit the lookup table for storing more total structure. Second, the rule learner is 50% more efficient ($I_"eff"^* = 0.0625$ vs.~$0.0417$) because it avoids the addressing overhead of a larger memory. Third, all three agents can be tied or separated on training-distribution accuracy, but only $C_u$ and $A$ jointly distinguish the rule learner as the agent with genuinely useful structure. Fourth, the noise memoriser stores one bit and dissipates the same energy as the rule learner, yet $C_u = 0$ --- it has learned nothing about viability. The framework captures this cleanly: useful structure is not raw stored information but relevance-filtered information about $Z^V$. Viewed through the information bottleneck, the rule learner implements the IB-optimal compression for this channel: it sits at the knee of the IB curve, achieving maximum $I(M; Z)$ at minimum encoding rate $I(M; O)$. The lookup table achieves the same $I(M; Z)$ at double the rate, placing it strictly inside the IB-achievable region @tishby1999.
...
The central claim is conceptual and operational: a thermodynamic definition of intelligence should include a usefulness filter on internal structure.

Several limits should be stated plainly. First, the measurement problem remains open. Equation @eq:cu is a concrete candidate, not a finished universal estimator. Some domains will require latent-state models; others will need predictive-control proxies. Second, the proposal is not a complete scalar ranking of systems. The relevant object is an intelligence profile: adaptive reach $A$, useful-structure stock $C_u$, and acquisition efficiency $I_"eff"$. Third, active inference already provides a rich account of viability and attracting states; the present contribution is narrower, namely an explicit efficiency term for the acquisition of relevance-filtered structure. Fourth, the framework does not require explicit symbolic world models. Implicit, embodied, and distributed representations can all count if they carry the right relevance-filtered distinctions. Fifth, when dissipated work approaches zero (near-reversible computation), $I_"eff"$ diverges; in practice the Landauer limit $k_B T ln 2$ per bit erasure provides a natural floor and the quantity remains well-defined for any physically realisable learner. Sixth, when viability is multi-dimensional, $C_u$ should be computed as the joint mutual information $I(M_t; Z_1^V, dots, Z_k^V | H_t)$; sums of per-criterion marginals can undercount synergistic structure.
...
The computation code used to generate @tab:toy and the neural-network validation suite will be made publicly available at #link("https://github.com/199-biotechnologies/fti")[github.com/199-biotechnologies/fti] upon publication. The scripts include `compute_cu.py` (exact toy calculation), `neural_experiment.py` (Level 2 experiment), `validation_suite.py` (7-agent cross-validation), and `hypothesis_battery.py` (40-hypothesis stress test).
